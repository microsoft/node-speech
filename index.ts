...
interface SpeechLib {
  transcribe: (modelPath: string, modelName: string, authTagHex: string, ivHex: string, cipherHex: string, callback: (error: Error | undefined, result: ITranscriptionResult) => void) => number,
...
export function transcribe({ modelPath, modelName, authTag, iv, cipher, signal }: ITranscriptionOptions, callback: ITranscriptionCallback): void {
  const id = speechapi.transcribe(modelPath, modelName, authTag.toString('hex'), iv.toString('hex'), cipher.toString('hex'), callback);
